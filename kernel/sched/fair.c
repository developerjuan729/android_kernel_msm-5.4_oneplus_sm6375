...
bool keep_slide_nolimit(struct task_struct *p)
{
	int cgrp_id = -1;

#ifdef CONFIG_CGROUP_SCHED
	cgrp_id = task_css(p, cpu_cgrp_id)->id;
#endif

	return (cgrp_id == CGROUP_TOP_APP) && p->slide_boost && is_heavy_ux_task(p);
}
#endif
...
int newidle_balance(struct rq *this_rq, struct rq_flags *rf)
{
	 ...
	 * early.
	 */
	if (oplus_rt_new_idle_balance(this_rq, sched_ktime_clock()) || this_rq->nr_running)
		goto out;
#endif
#endif
